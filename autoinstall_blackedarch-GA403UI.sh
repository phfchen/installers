...
if [[ $PKGS =~ ^[Yy]$ ]]; then
    ...
    app_pkgs="openvpn vlc zathura zathura-pdf-mupdf zathura-ps"
    util_pkgs="brightnessctl cifs-utils fzf gparted gvfs-nfs gvfs-smb network-manager-applet networkmanager-openvpn neofetch nfs-utils pacman-contrib power-profiles-daemon python-pip rust-script slurp smblient thunar thunar-archive-plugin thunar-volman thunar-media-tas-plugins vlc-plugin-ffmpeg tumbler trash-cli unzip usbutils"
    font_pkgs="noto-fonts noto-fonts-cjk noto-fonts-emoji"
    theme_pkgs=""
    extra_pkgs="brave-bin mullvad-browser-bin gimp joplin-desktop libreoffice"
...
if [[ $GITCFG =~ ^[Yy]$ ]]; then
    ...
    mkdir -p ~/.config ~/Desktop ~/Documents ~/Downloads ~/Pictures ~/Videos ~/Temp 2>&1 | tee -a $LOG
...
if [[ $ASUS =~ ^[Yy]$ ]]; then
    ...
    asus_pkgs="asusctl rog-control-center"
...
if [[ $WLAN8814AU =~ ^[Yy]$ ]]; then
    ...
    if ! $aur -S --noconfirm --needed $wlan8814au_pkgs 2>&1 | tee -a $LOG; then
        ...
        printf " Cloning morrownr git packages...\n"
        mkdir -p ~/Documents/git 2>&1 | tee -a $LOG
        cd ~/Documents/git 2>&1 | tee -a $LOG
        git clone https://github.com/morrownr/8814au.git 2>&1 | tee -a $LOG
        cd 8814au 2>&1 | tee -a $LOG
        printf " Installing 8814AU WLAN driver"
        sudo ./install-driver.sh 2>&1 | tee -a $LOG
        sleep 1
    fi
else
